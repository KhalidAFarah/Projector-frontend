...
import { HoverWeapon } from "./HoverWeapon";
...
const Build = (props:any) => {
  ...
  const [kineticHover, setKineticHover] = useState(false);
        ...
        <Row>
          <Col sm={3} md={3} lg={3} style={{paddingLeft: "5%" , paddingTop:"20%"}}>
            ...
            <Row>
              ...
              <Image style={{width:"100%", margin:"2%", transform: "scale(1.2)"}} src={displayable.subclass}></Image>
            </Row>
            <Row style={{marginTop: "30%", marginBottom: "15%"}} onPointerOver={()=>{setKineticHover(true)}} onPointerLeave={()=>{setKineticHover(false)}}>
            {kineticHover ? <HoverWeapon item={props.build.kinetic}/> : ""}
              <Image style={{width:"100%"}} src={displayable.kinetic} ></Image>
            </Row>
            <Row>
            {energyHover ? <HoverWeapon item={props.build.energy} /> : ""}
              <Image style={{width:"100%", marginBottom: "15%"}} src={displayable.energy} onPointerOver={()=>{setEnergyHover(true)}} onPointerLeave={()=>{setEnergyHover(false)}}></Image>
            </Row>
            <Row>
            {heavyHover ? <HoverWeapon item={props.build.heavy} /> : ""}
              <Image style={{width:"100%"}} src={displayable.heavy} onPointerOver={()=>{setHeavyHover(true)}} onPointerLeave={()=>{setHeavyHover(false)}}></Image>
            </Row>
          </Col >
